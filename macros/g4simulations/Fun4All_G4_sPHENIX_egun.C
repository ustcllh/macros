...
int Fun4All_G4_sPHENIX_egun(
    const int nEvents = 5,
    const char *inputFile = "/sphenix/data/data02/review_2017-08-02/single_particle/spacal2d/fieldmap/G4Hits_sPHENIX_e-_eta0_8GeV-0002.root",
    const char *outputFile = "G4sPHENIX.root",
    const char *embed_input_file = "/sphenix/data/data02/review_2017-08-02/sHijing/fm_0-4.list",
    const char *input_particle = "pi-",
    const int input_energy = 8,
    const double input_eta = 0.2)
{
...
//            PHG4SimpleEventGenerator::Uniform);
    ...
    if (particles)
    {
      ...
      gen->add_particles(input_particle, 1);  // mu+,e+,proton,pi+,Upsilon
      ...
      gen->set_eta_range(input_eta, input_eta);
    ...
    if (usegun)
    {
      ...
      pgen->set_name(input_particle);
      pgen->set_z_range(0, 0);
      pgen->set_eta_range(input_eta, input_eta);
      pgen->set_mom_range(input_energy, input_energy);
  ...
  MyTreeMaker *tm = new MyTreeMaker( outputFile ,true,true,false,false,false,false,false,false);
