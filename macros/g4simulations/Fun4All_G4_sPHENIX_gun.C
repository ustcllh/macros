...
int Fun4All_G4_sPHENIX_gun(
    const int nEvents = 1,
    const char *inputFile = "/sphenix/data/data02/review_2017-08-02/single_particle/spacal2d/fieldmap/G4Hits_sPHENIX_e-_eta0_8GeV-0002.root",
    const char *outputFile = "G4sPHENIX.root",
    const char *embed_input_file = "/sphenix/data/data02/review_2017-08-02/sHijing/fm_0-4.list",
    const char *input_particle = "e-",
    const int input_energy = 2)
{
  ...
  gSystem->Load("libMyTreeMaker.so");
  ...
  else
  {
    ...
    if (runpythia8)
    {
      ...
      //pythia8->set_config_file("phpythia8_QCD15.cfg"); //dijet
      //pythia8->set_config_file("phpythia8_QCD15_prompt.cfg"); //gammajet
...
//            PHG4SimpleEventGenerator::Uniform);
      ...
      jettrig->SetEtaHighLow(jetmaxeta, -jetmaxeta);
      //jettrig->SetMinJetPt(5.0); //gammajet
      //jettrig->SetMinJetPt(15.0); //dijet
      jettrig->SetMinJetPt(jetminpt); //dijet 40 GeV
      jettrig->SetMinLeadingZ(jetminz);
      jettrig->PrintConfig();
    ...
    if (particles)
    {
      ...
      //gen->add_particles(input_particle, 5);  // mu+,e+,proton,pi+,Upsilon
      gen->add_particles(130,1); // k0 long
      ...
      gen->set_eta_range(0.0, 0.6);
      gen->set_phi_range(-1.0 * TMath::Pi(), 1.0 * TMath::Pi());
      //gen->set_pt_range(0.1, 50.0);
      gen->set_p_range(input_energy, input_energy);
      gen->Embed(1);
      gen->Verbosity(0);
    ...
    if (usegun)
    {
      ...
      pgen->set_mom_range(10, 30);
      pgen->set_phi_range(0. * TMath::Pi(), 2. * TMath::Pi());
  ...
  //MyTreeMaker *tm = new MyTreeMaker( outputFile ,true,true,true,true,false,false,false);
  //MyTreeMaker *tm = new MyTreeMaker( outputFile ,true,true,true,true,true,true,true);
  MyTreeMaker *tm = new MyTreeMaker( outputFile ,true,false,false,true,false,false,false);
