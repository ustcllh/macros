...
int Fun4All_G4_sPHENIX_PFJet(
    const int nEvents = 1,
    const double jetmaxeta = 0.45,
    const double jetminpt = 30.0,
    const double jetminz = 0.8,
    ...
    const char *embed_input_file = "/sphenix/data/data02/review_2017-08-02/sHijing/fm_0-4.list")
{
  ...
  const string magfield = "/sphenix/user/decadal/fieldmaps/sPHENIX.2d.root";
...
//            PHG4SimpleEventGenerator::Uniform);
  ...
  if (do_PHFJet)
  {
    PHFlowJetMaker *pfjet_ak_r4 = new PHFlowJetMaker("pfjet_ak_r4", "AntiKt", 0.4);
    se->registerSubsystem(pfjet_ak_r4);
  }
  //----------------------
  ...
  MyTreeMaker *tm = new MyTreeMaker( outputFile ,true,true,false,false,true,false,true,false);
  // truth particles / track / tower / cluster / truth jet / tower jet / flow jet / hits
